...
def process_data(df):
    """
    处理数据：找到关键列、转换数据类型、排序
    
    Args:
        df: 原始DataFrame
    
    Returns:
        tuple: (处理后的DataFrame, ratio_col, periods, period_tables)
    """
    ...
    return df_total, ratio_col, periods, period_tables


def write_report_file(df_total, ratio_col, periods, period_tables, years: list, output_dir: str = "output"):
    """
    写入报告文件
    
    Args:
        df_total: 总排名DataFrame
        ratio_col: 比例列名
        periods: 报告期列表
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("=" * 100 + "\n")
        if len(years) == 1:
            f.write(f"总排名表（{years[0]}年所有报告期数据汇总）\n")
        else:
            f.write("总排名表（所有报告期数据汇总）\n")
        f.write("=" * 100 + "\n\n")
...
def print_report_summary(df_total, ratio_col, periods, period_tables, years: list):
    """
    在控制台打印报告摘要
...
def generate_report(years: list, output_dir: str = "output"):
    ...
    # 处理数据
    df_total, ratio_col, periods, period_tables = process_data(df)
    ...
    # 写入文件
    output_file = write_report_file(df_total, ratio_col, periods, period_tables, years, output_dir)
    print(f"\n报告已保存到: {output_file}")
    
    # 打印摘要
    print_report_summary(df_total, ratio_col, periods, period_tables, years)
